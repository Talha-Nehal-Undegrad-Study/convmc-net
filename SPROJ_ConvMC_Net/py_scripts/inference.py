...
def make_and_store_predictions(model_dict_path, q, sigma, params_net, hyper_param_net, train_loader, val_loader, device, SESSION):
    ...
    if hyper_param_net['Model'] == 'ADMM-Net':
      model = UnfoldedNet3dC_admm(params_net)
    else:
...
def search_and_save_best_model(SESSION, params_net, q, sigma, model, device):
    ...
    # make_and_store_predictions(best_model_path, q, sigma, params_net, hyper_param_net, train_loader, val_loader, device, SESSION) --> not run due to low space
